...
import { fetchLatestCloudSensorData, setDeviceProperty } from "@/services/onenet-cloud";
...
/**
 * POST /api/v1/sensors/wifi-config
 * 设置设备 WiFi 配置（上传到 OneNet）
 * Body: { ssid: string, password: string, deviceId?: string }
 */
router.post("/wifi-config", async (req: Request, res: Response) => {
  try {
    const { ssid, password, deviceId } = req.body;

    if (!ssid || !password) {
      return res.status(400).json({ error: "ssid 和 password 不能为空" });
    }

    // 调用 OneNet API 设置设备属性
    const result = await setDeviceProperty(
      {
        ssid: ssid,
        sspassword: password,
      },
    ...
    if (result.success) {
      res.json({ success: true, message: "WiFi 配置已下发，设备将自动连接" });
    } else {
      res.status(500).json({ error: result.message });
    }
  } catch (error) {
    console.error("Set WiFi config error:", error);
    res.status(500).json({ error: "Internal server error" });
  }
});
