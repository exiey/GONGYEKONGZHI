/**
 * Copyright (c), 2012~2024 iot.10086.cn All Rights Reserved
 *
 * @file tcp_linux.c
 * @brief TCP API for linux
 */

/*****************************************************************************/
/* Includes                                                                  */
/*****************************************************************************/
#include <arpa/inet.h>
#include <errno.h>
#include <fcntl.h>
#include <netdb.h>
#include <netinet/in.h>
#include <stdbool.h>
#include <stdlib.h>
#include <string.h>
#include <sys/socket.h>
#include <sys/time.h>
#include <unistd.h>

#include "data_types.h"
#include "err_def.h"
#include "log.h"
#include "plat_osl.h"
#include "plat_tcp.h"
#include "stdio.h"

/*****************************************************************************/
/* Local Definitions ( Constant and Macro )                                  */
/*****************************************************************************/

/*****************************************************************************/
/* Structures, Enum and Typedefs                                             */
/*****************************************************************************/

/*****************************************************************************/
/* Local Function Prototype                                                  */
/*****************************************************************************/

static boolean check_connect(int32_t fd, uint32_t timeout_ms);

/*****************************************************************************/
/* Local Variables                                                           */
/*****************************************************************************/

/*****************************************************************************/
/* Global Variables                                                          */
/*****************************************************************************/

/*****************************************************************************/
/* Function Implementation                                                   */
/*****************************************************************************/
#include "common.h"
#ifndef AIOT_HEX_PRINT
#define AIOT_HEX_PRINT(tag, buf, len)                                        \
  do {                                                                       \
    if (buf == NULL || len == 0) {                                           \
      break;                                                                 \
    }                                                                        \
                                                                             \
    /* 作用域隔离，避免变量名冲突 */                            \
    {                                                                        \
      char *hex_log_buf = NULL; /* 存储十六进制日志的缓冲区 */   \
      uint32_t hex_buf_len =                                                 \
          len * 3 + 1; /* 每个字节占3字符（"XX "）+ 终止符 */    \
      uint8_t *data_ptr = (uint8_t *)buf;                                    \
      uint32_t write_offset = 0;                                             \
      int snprintf_ret = 0;                                                  \
                                                                             \
      SAFE_ALLOC(hex_log_buf, hex_buf_len);                                  \
                                                                             \
      for (uint32_t i = 0; i < len; i++) {                                   \
        snprintf_ret =                                                       \
            snprintf(hex_log_buf + write_offset, hex_buf_len - write_offset, \
                     "%02X ", data_ptr[i]);                                  \
                                                                             \
        if (snprintf_ret < 0) {                                              \
          loge("%s: HEX_PRINT snprintf failed at index %d", tag, i);         \
          SAFE_FREE(hex_log_buf);                                            \
          break;                                                             \
        }                                                                    \
        write_offset += snprintf_ret;                                        \
      }                                                                      \
                                                                             \
      /* 移除最后一个多余的空格（若有） */                    \
      if (write_offset > 0 && hex_log_buf[write_offset - 1] == ' ') {        \
        hex_log_buf[write_offset - 1] = '\0';                                \
      }                                                                      \
                                                                             \
      logd("%s: %s", tag, hex_log_buf);                                      \
      SAFE_FREE(hex_log_buf);                                                \
    }                                                                        \
  } while (0)
#endif

handle_t plat_tcp_connect(const uint8_t *host, uint16_t port,
                          uint32_t timeout_ms) {
  handle_t fd = -1;
  struct sockaddr_in addr_in;
  struct hostent *ip_addr;
  int32_t flags = -1;
  int ret = -1;

  if (0 == (ip_addr = gethostbyname(host))) {
    goto exit;
  }

  addr_in.sin_family = AF_INET;
  addr_in.sin_addr = *((struct in_addr *)ip_addr->h_addr_list[0]);
  addr_in.sin_port = htons(port);

  if (0 > (fd = socket(AF_INET, SOCK_STREAM, 0))) {
    goto exit;
  }

  if ((0 > (flags = fcntl(fd, F_GETFL, 0))) ||
      (0 > fcntl(fd, F_SETFL, flags | O_NONBLOCK))) {
    goto close_socket;
  }

  if (0 > (ret = connect(fd, (struct sockaddr *)&addr_in, sizeof(addr_in)))) {
    if (EINPROGRESS != errno) {
      goto close_socket;
    }
  }
  if (check_connect(fd, timeout_ms)) return fd;

close_socket:
  close(fd);
exit:
  return -1;
}

int32_t plat_tcp_send(handle_t handle, void *buf, uint32_t len,
                      uint32_t timeout_ms) {
  handle_t countdown_tmr = 0;
  struct timeval tv;
  fd_set fs;
  int sent_len = 0;
  int ret = 0;
  uint32_t left_time = 0;

  if ((0 > handle) || (0 == (countdown_tmr = countdown_start(timeout_ms))))
    return -1;

  do {
    left_time = countdown_left(countdown_tmr);
    tv.tv_sec = left_time / 1000;
    tv.tv_usec = (left_time % 1000) * 1000;

    FD_ZERO(&fs);
    FD_SET(handle, &fs);

    ret = select(handle + 1, NULL, &fs, NULL, &tv);
    if (0 < ret) {
      if (FD_ISSET(handle, &fs)) {
        ret = send(handle, buf + sent_len, len - sent_len, MSG_DONTWAIT);
        if (0 < ret) {
          sent_len += ret;
        } else if (0 > ret) {
          sent_len = ret;
          break;
        }
      }
    } else if (0 > ret) {
      sent_len = ret;
      break;
    }
  } while ((sent_len < len) && (0 == countdown_is_expired(countdown_tmr)));
  countdown_stop(countdown_tmr);

  return sent_len;
}

int32_t plat_tcp_recv(handle_t handle, void *buf, uint32_t len,
                      uint32_t timeout_ms) {
  handle_t countdown_tmr = 0;
  struct timeval tv;
  fd_set fs;
  int recv_len = 0;
  int ret = 0;
  uint32_t left_time = 0;

  if ((0 > handle) || (0 == (countdown_tmr = countdown_start(timeout_ms))))
    return -1;

  do {
    left_time = countdown_left(countdown_tmr);

    tv.tv_sec = left_time / 1000;
    tv.tv_usec = (left_time % 1000) * 1000;

    FD_ZERO(&fs);
    FD_SET(handle, &fs);

    ret = select(handle + 1, &fs, NULL, NULL, &tv);
    if (0 < ret) {
      if (FD_ISSET(handle, &fs)) {
        ret = recv(handle, buf + recv_len, len - recv_len, MSG_DONTWAIT);

        recv_len += ret;
      }
    }
  } while ((recv_len < len) && (0 == countdown_is_expired(countdown_tmr)));
  countdown_stop(countdown_tmr);

  //   if (recv_len > 0) {
  //     logi("recv_len=%d", recv_len);
  //   }

  //AIOT_HEX_PRINT("TCP Recv", buf, recv_len);
  return recv_len;
}

int32_t plat_tcp_disconnect(handle_t handle) {
  if (0 < handle) {
    close(handle);
  }

  return 0;
}

static boolean check_connect(int32_t fd, uint32_t timeout_ms) {
  struct timeval tv;
  fd_set fs;
  socklen_t len = 0;
  int32_t ret = 0;
  int32_t sock_error = 0;

  tv.tv_sec = timeout_ms / 1000;
  tv.tv_usec = (timeout_ms % 1000) * 1000;

  FD_ZERO(&fs);
  FD_SET(fd, &fs);
  ret = select(fd + 1, NULL, &fs, NULL, &tv);
  if (0 < ret) {
    if (FD_ISSET(fd, &fs)) {
      len = sizeof(sock_error);
      if ((0 == getsockopt(fd, SOL_SOCKET, SO_ERROR, &sock_error, &len)) &&
          (0 == sock_error)) {
        return TRUE;
      }
    }
  }

  return FALSE;
}