...
{
    // SPI初始化由CubeMX生成，这里只需要初始化GPIO
    GPIO_InitTypeDef GPIO_InitStructure = {0};

    // 使能GPIOA时钟
    __HAL_RCC_GPIOA_CLK_ENABLE();

    // 配置PA5(SCLK)和PA7(MOSI)为复用推挽输出
...
void SPI_OLED_Init(void)
{
    ...
    HAL_GPIO_WritePin(GPIOB, GPIO_PIN_0 | GPIO_PIN_1 | GPIO_PIN_2, GPIO_PIN_SET); // PB0,PB1,PB2输出高
